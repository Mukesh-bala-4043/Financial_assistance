...
def binfo():
    try:
        cr = conn.cursor()
        cr.execute(f"select sum(tamt) from Credit where tdate like '____-{str(d.month)}-%' ")
        Usd = cr.fetchall()[0][0]
        cr.execute(f"select sum(tamt) from Debit where tdate like '____-{str(d.month)}-%'  ")
        Bal = cr.fetchall()[0][0] - Usd
        cr.close()
    except:
        Usd, Bal = 0, 0
    return Usd, Bal
